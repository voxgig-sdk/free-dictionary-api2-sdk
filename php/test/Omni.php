...
//    only - a typed FreeDictionaryApi2Context there would make every ctx
...
final class FreeDictionaryApi2Omni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new FreeDictionaryApi2Context($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new FreeDictionaryApi2Spec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new FreeDictionaryApi2Result($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new FreeDictionaryApi2Error('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new FreeDictionaryApi2Response($clean['response']);
    ...
    public static function err_from_map($m): ?FreeDictionaryApi2Error
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FreeDictionaryApi2Error(is_string($code) ? $code : '', $msg);
    }
