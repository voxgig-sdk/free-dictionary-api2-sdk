...
class LanguageEntity
{
    ...
    {
        if ($args) {
            $this->_data = FreeDictionaryApi2Helpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = FreeDictionaryApi2Helpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws FreeDictionaryApi2Error on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FreeDictionaryApi2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
