...
_runner = makeRunner(TEST_JSON_FILE, FreeDictionaryApi2SDK.test(None, None))
...
def _auth_credential():
    ...
    ctx.spec = FreeDictionaryApi2Spec({"headers": {}, "query": {}})
    try:
...
class TestPrimaryUtility:
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = FreeDictionaryApi2SDK.test(None, setup.get('a') or {})
    ...
    def test_operator_basic(self):
        def subject(vin):
            if not isinstance(vin, dict):
                vin = {}
            op = FreeDictionaryApi2Operation(vin)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = FreeDictionaryApi2SDK.test(None, setup.get('a') or {})
