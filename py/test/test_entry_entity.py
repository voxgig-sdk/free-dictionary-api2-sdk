    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FREEDICTIONARYAPI__TEST_ENTRY_ENTID JSON to run live")
        client = setup["client"]
        ...
        entry_ref01_data_dt0_loaded, err = entry_ref01_ent.load(entry_ref01_match_dt0, None)
        assert err is None
...
def _entry_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FREEDICTIONARYAPI__TEST_ENTRY_ENTID"))
    ...
    _live = env.get("FREEDICTIONARYAPI__TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FREEDICTIONARYAPI__TEST_EXPLAIN") == "TRUE",
        "live": _live,
