    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FREE_DICTIONARY_API2_TEST_ENTRY_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        entry_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.entry")))
        entry_ref01_data = None
        if len(entry_ref01_data_raw) > 0:
...
def _entry_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "FREE_DICTIONARY_API2_TEST_ENTRY_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FREE_DICTIONARY_API2_TEST_ENTRY_ENTID"))
    ...
    if env.get("FREE_DICTIONARY_API2_TEST_LIVE") == "TRUE":
        ...
        client = FreeDictionaryApi2SDK(helpers.to_map(merged_opts))

    _live = env.get("FREE_DICTIONARY_API2_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FREE_DICTIONARY_API2_TEST_EXPLAIN") == "TRUE",
        "live": _live,
