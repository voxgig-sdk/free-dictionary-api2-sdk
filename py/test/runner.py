# FreeDictionaryApi2 SDK test runner
...
class FreeDictionaryApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeDictionaryApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeDictionaryApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeDictionaryApi2TestRunner.getenv("FREE_DICTIONARY_API2_TEST_LIVE")
        override = FreeDictionaryApi2TestRunner.getenv("FREE_DICTIONARY_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeDictionaryApi2TestRunner.getenv(key)
        ...
        explain = FreeDictionaryApi2TestRunner.getenv("FREE_DICTIONARY_API2_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREE_DICTIONARY_API2_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeDictionaryApi2TestRunner._test_control is not None:
            return FreeDictionaryApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeDictionaryApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeDictionaryApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeDictionaryApi2TestRunner.load_test_control()
...
def load_env_local():
    FreeDictionaryApi2TestRunner.load_env_local()


def env_override(m):
    return FreeDictionaryApi2TestRunner.env_override(m)


def entity_data(v):
    return FreeDictionaryApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeDictionaryApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeDictionaryApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeDictionaryApi2TestRunner.load_test_control()


def live_client_options():
    return FreeDictionaryApi2TestRunner.live_client_options()


def live_delay_ms():
    return FreeDictionaryApi2TestRunner.live_delay_ms()
