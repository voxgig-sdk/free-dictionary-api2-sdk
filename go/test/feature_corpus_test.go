...
//
// NewFreeDictionaryApi2SDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.FreeDictionaryApi2SDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewFreeDictionaryApi2SDK(opts)
}
...
func fcCandidates(client *sdk.FreeDictionaryApi2SDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.FreeDictionaryApi2SDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.FreeDictionaryApi2SDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.FreeDictionaryApi2Error
