...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FREE_DICTIONARY_API2_TEST_LIVE") == "TRUE" ||
		os.Getenv("FREE_DICTIONARY_API2_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FREE_DICTIONARY_API2_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.FreeDictionaryApi2SDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.FreeDictionaryApi2SDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.FreeDictionaryApi2Error{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.FreeDictionaryApi2SDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.FreeDictionaryApi2Error{Msg: msg, Code: code}
}
