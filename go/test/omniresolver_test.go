...
import (
	...
	omni "github.com/voxgig-sdk/free-dictionary-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeDictionaryApi2Error); is {
				out := map[string]any{
					"name":    "FreeDictionaryApi2Error",
...
func omniCtx(arg any, client *sdk.FreeDictionaryApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeDictionaryApi2SDK); is {
			client = live
