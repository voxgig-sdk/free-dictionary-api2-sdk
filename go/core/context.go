...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*FreeDictionaryApi2SDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *FreeDictionaryApi2Error {
	return NewFreeDictionaryApi2Error(code, msg, ctx)
}
