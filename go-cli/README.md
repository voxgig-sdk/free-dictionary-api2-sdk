# free-dictionary-api2-cli

AQL-driven command-line client **and** interactive REPL for the FreeDictionaryApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-dictionary-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./free-dictionary-api2-cli --help
...
./free-dictionary-api2-cli load 1 entry            # {id:1} shorthand
./free-dictionary-api2-cli load '{id:1}' entry       # explicit match map

# 5. Override the API base URL for a single call
FREE_DICTIONARY_API2_BASE=https://api.example.com ./free-dictionary-api2-cli load 1 entry

# 6. No arguments -> interactive REPL
./free-dictionary-api2-cli
free-dictionary-api2> load 1 entry
free-dictionary-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-dictionary-api2-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-dictionary-api2-cli load 1 entry
   ```
...
```sh
./free-dictionary-api2-cli load 1 entry          # scalar shorthand for {id:1}
./free-dictionary-api2-cli load '{id:1}' entry     # explicit match map
```
...
./free-dictionary-api2-cli load 1 entry
```
...
Run with no arguments to open a REPL (prompt `free-dictionary-api2>`). Each line is
evaluated as its own AQL expression:

```text
$ ./free-dictionary-api2-cli
free-dictionary-api2> load 1 entry
free-dictionary-api2> /help
free-dictionary-api2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `entry`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-dictionary-api2-cli`. |
...
(`./free-dictionary-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
