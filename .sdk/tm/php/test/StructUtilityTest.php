...
// FreeDictionaryApi2 SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FreeDictionaryApi2Omni::makeStructRunner(
    ...
    {
        return FreeDictionaryApi2Omni::objmodel(FreeDictionaryApi2Omni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return FreeDictionaryApi2Omni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return FreeDictionaryApi2Omni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FreeDictionaryApi2Omni::structfix($now, Struct::undef()))
                        !== json_encode(FreeDictionaryApi2Omni::structfix($val, Struct::undef()))) {
