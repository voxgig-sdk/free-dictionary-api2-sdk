# FreeDictionaryApi2 SDK utility: fetcher
...
module FreeDictionaryApi2Utilities
  ...
  DefaultHttpFetch = ->(fullurl, fetchdef) {
    ...
    begin
      ...
      request['User-Agent'] = 'Mozilla/5.0 (compatible; FreeDictionaryApi2SDK/1.0)' unless has_ua
      request.body = body_str if body_str.is_a?(String)

      http = FreeDictionaryApi2Utilities.http_checkout(key, uri, proxy)
      begin
        # Net::HTTP retries idempotent requests; an EOF can follow a committed POST.
        resp = http.request(request)
      rescue StandardError
        FreeDictionaryApi2Utilities.http_discard(http)
        raise
      end
      FreeDictionaryApi2Utilities.http_checkin(key, http)
