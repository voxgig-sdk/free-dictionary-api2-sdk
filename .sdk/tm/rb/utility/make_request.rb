...
module FreeDictionaryApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeDictionaryApi2Response.new({})
    ...
    if fetch_err
      ...
      response = FreeDictionaryApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeDictionaryApi2Response.new(fetched)
    else
