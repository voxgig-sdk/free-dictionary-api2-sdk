...
//     (\`new FreeDictionaryApi2SDK(...)\` / \`FreeDictionaryApi2SDK.test(...)\` become
//     \`FreeDictionaryApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FreeDictionaryApi2SDK'
...
const TEST_SEED = {"entity":{"entry":{"test01":{"id":"test01"}},"language":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeDictionaryApi2SDK(...)\` and any
// \`FreeDictionaryApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeDictionaryApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeDictionaryApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
