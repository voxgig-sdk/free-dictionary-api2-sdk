...
describe('LanguageEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    const live = 'TRUE' === process.env.FREE_DICTIONARY_API__TEST_LIVE
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set FREE_DICTIONARY_API__TEST_LANGUAGE_ENTID JSON to run live')
      return
    }
    ...
    const language_ref01_data_dt0 = await language_ref01_ent.load(language_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['FREE_DICTIONARY_API__TEST_LANGUAGE_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'FREE_DICTIONARY_API__TEST_LANGUAGE_ENTID': idmap,
    'FREE_DICTIONARY_API__TEST_LIVE': 'FALSE',
    'FREE_DICTIONARY_API__TEST_EXPLAIN': 'FALSE',
    'FREE_DICTIONARY_API__APIKEY': 'NONE',
  })

  idmap = env['FREE_DICTIONARY_API__TEST_LANGUAGE_ENTID']

  const live = 'TRUE' === env.FREE_DICTIONARY_API__TEST_LIVE
  ...
  const setup = {
    ...
    struct,
    data: entityData,
    explain: 'TRUE' === env.FREE_DICTIONARY_API__TEST_EXPLAIN,
    live,
