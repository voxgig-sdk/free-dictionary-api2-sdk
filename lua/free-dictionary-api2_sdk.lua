...
require("free-dictionary-api2_types")
...
local FreeDictionaryApi2SDK = {}
FreeDictionaryApi2SDK.__index = FreeDictionaryApi2SDK
...
FreeDictionaryApi2SDK._make_feature = _make_feature


function FreeDictionaryApi2SDK.new(options)
  local self = setmetatable({}, FreeDictionaryApi2SDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FreeDictionaryApi2SDK:options_map()
...
function FreeDictionaryApi2SDK:get_utility()
  return Utility.copy(self._utility)
end


function FreeDictionaryApi2SDK:get_root_ctx()
  return self._rootctx
end


function FreeDictionaryApi2SDK:prepare(fetchargs)
...
function FreeDictionaryApi2SDK:direct(fetchargs)
...
function FreeDictionaryApi2SDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function FreeDictionaryApi2SDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "FreeDictionaryApi2SDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function FreeDictionaryApi2SDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FreeDictionaryApi2SDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FreeDictionaryApi2SDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FreeDictionaryApi2SDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FreeDictionaryApi2SDK.new(sdkopts)
  sdk.mode = "test"
