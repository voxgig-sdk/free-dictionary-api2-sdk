...
The Lua SDK for the FreeDictionaryApi2 API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Entry(nil):load({ id = "example_id" }, nil)
...
local result, err = client:FreeDictionaryApi2(nil):load(
  { id = "test01" }, nil
)
