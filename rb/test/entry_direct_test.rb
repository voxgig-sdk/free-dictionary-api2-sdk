...
class EntryDirectTest < Minitest::Test
  def test_direct_load_entry
    setup = entry_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-entry", setup[:live] ? "live" : "unit")
    if _should_skip
...
def entry_direct_setup(mockres)
