...
class EntryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FREE_DICTIONARY_API2_TEST_ENTRY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    entry_ref01_data_dt0_load_result = Helpers.to_map(entry_ref01_data_dt0_loaded.respond_to?(:data_get) ? entry_ref01_data_dt0_loaded.data_get : entry_ref01_data_dt0_loaded)
    assert !entry_ref01_data_dt0_load_result.nil?
    assert_equal entry_ref01_data_dt0_load_result["id"], entry_ref01_data["id"]
...
def entry_basic_setup(extra)
  ...
  client = FreeDictionaryApi2SDK.test(options, extra)
  ...
  entid_env_raw = ENV["FREE_DICTIONARY_API2_TEST_ENTRY_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["FREE_DICTIONARY_API2_TEST_ENTRY_ENTID"])
  ...
  if env["FREE_DICTIONARY_API2_TEST_LIVE"] == "TRUE"
    ...
    client = FreeDictionaryApi2SDK.new(Helpers.to_map(merged_opts))
  end
  ...
  live = env["FREE_DICTIONARY_API2_TEST_LIVE"] == "TRUE"
  {
    ...
    env: env,
    explain: env["FREE_DICTIONARY_API2_TEST_EXPLAIN"] == "TRUE",
    live: live,
