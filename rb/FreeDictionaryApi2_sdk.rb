...
require_relative 'FreeDictionaryApi2_types'


class FreeDictionaryApi2SDK
  ...
  def initialize(options = {})
    ...
    utility = FreeDictionaryApi2Utility.new
    @_utility = utility
    ...
    feature_opts = FreeDictionaryApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreeDictionaryApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreeDictionaryApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreeDictionaryApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreeDictionaryApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreeDictionaryApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreeDictionaryApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FreeDictionaryApi2Error.new(
        "#{op}_allow",
        "FreeDictionaryApi2SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FreeDictionaryApi2Error => err
    ...
    if fetched.is_a?(Hash)
      status = FreeDictionaryApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreeDictionaryApi2Error.new(
        "graphql_error", "FreeDictionaryApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FreeDictionaryApi2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
