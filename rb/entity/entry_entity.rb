# FreeDictionaryApi2 SDK Entry entity
...
class EntryEntity
  ...
  def make
    opts = @_entopts.dup
    EntryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FreeDictionaryApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Entry, Hash] the current Entry data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FreeDictionaryApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Entry fields)
  def match_get
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FreeDictionaryApi2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
